...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        HashMap<Integer, String> revealNeuronName = new HashMap<>();

        for(int i=0; i<neurons; i++) {
            String r = StdIn.readString();
            neuronName.put(r, i);
            revealNeuronName.put(i, r);
        }

        int synapses = StdIn.readInt();
        int[][] directSynapses = new int[neurons][neurons];

        for(int i=0; i<synapses; i++) {
            String from = StdIn.readString();
            String to = StdIn.readString();
            int fromRow = neuronName.get(from);
            int toColumn = neuronName.get(to);

            directSynapses[fromRow][toColumn] = 1;
        }
        ...
        for(int i=0; i<neurons; i++) {
            if(directSynapses[i][nillOutDegree] == 1){
                StdOut.println(revealNeuronName.get(i));
            }
        }
     }
}
