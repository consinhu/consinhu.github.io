...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
                                     ...
                                     double[][] drywet, double[][] wetwet){
                                        ...
                                        for (int o = 0; o < 12; o++)
                                        {
                                            wetwetProbability[o] = wetwet[numberOfRow][o + 2];
                                        }
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        double initial = StdRandom.uniform();
        ...
        if (initial < .5)
            ...
            for(int a = 1; a < forecast.length; a++)
            {
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude ){
        ...
        int[] monthForecast = forecastGenerator(drywetProbability[month], wetwetProbability[month], numberOfDaysInMonth[month]);
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        for(int i = 0; i < forecast.length; i++) {
            ...
            if(max < count && forecast[i-1] == mode) {
                max = count;
            } 
        ...
        if(max < count && forecast[forecast.length-1] == mode) {
            max = count;
        }
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        for(int i = 0; i < forecast.length - 6; i++) {
            for(int j = 0; j < array.length; j++) {
                array[j] = forecast[i + j];
            }

            count = dryStreak;
            dryStreak = numberOfWetDryDays(array, 2);

            if(dryStreak > count) {
                index = i;
            } else if(count > dryStreak) {
                dryStreak = count;
            } else {
                dryStreak = count;
            }
