public class LeasingCost {
   ...
   public static Vehicle createVehicle(String description){
       String temp = description.substring(description.indexOf("type:")+5);
       String type = temp.substring(0, temp.indexOf(";"));

       temp = description.substring(description.indexOf("name:")+5);
       String name = temp.substring(0, temp.indexOf(";"));

       temp = description.substring(description.indexOf("due:")+4);
       String due = temp.substring(0, temp.indexOf(";"));

       temp = description.substring(description.indexOf("length:")+7);
       String length = temp.substring(0, temp.indexOf(";"));

       temp = description.substring(description.indexOf("monthly:")+8);
       String monthly = temp.substring(0, temp.indexOf(";"));

       temp = description.substring(description.indexOf("mile/unit:")+10);
       String mileUnit = temp.substring(0, temp.indexOf(";"));

       temp = description.substring(description.indexOf("allowance:")+10);
       String mileageAllowance = temp.substring(0, temp.indexOf(";"));

       Lease newLease = new Lease(Double.parseDouble(due), Integer.parseInt(length), Double.parseDouble(monthly), Integer.parseInt(mileageAllowance));
       Fuel newFuel;

       if(type.equals("electric")) {
           temp = description.substring(description.indexOf("charger:")+8);
           String charger = temp.substring(0, temp.indexOf(";"));
           newFuel = new Fuel(Double.parseDouble(mileUnit), Double.parseDouble(charger));
       }
       else {
           newFuel = new Fuel(Double.parseDouble(mileUnit));
       }

       Vehicle newVehicle = new Vehicle(name, newFuel, newLease);
       return newVehicle;
   }
   ...
   public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
       ...
       for(Vehicle vehicle : vehicles) {
           if(vehicle.getFuel().getType() == 1) {
               vehicle.setCO2Emission(computeCO2(vehicle.getLease().getLeaseLength(), vehicle.getFuel().getUsage(), vehicle.getLease().getMileageAllowance(), Fuel.CO2EMITTED_GASCOMBUSTION));
               ...
               vehicle.setTotalCost(computeLeaseCost(vehicle.getLease().getDueAtSigning(), vehicle.getLease().getLeaseLength(), vehicle.getLease().getMonthlyCost()) + vehicle.getFuelCost());
           } else {
               ...
               vehicle.setTotalCost(computeLeaseCost(vehicle.getLease().getDueAtSigning(), vehicle.getLease().getLeaseLength(), vehicle.getLease().getMonthlyCost()) + vehicle.getFuelCost() + vehicle.getFuel().getCharger());
           }
       }
}
